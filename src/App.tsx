...
const contextMap: Record<number, Record<number, string | undefined>> = {
  1: {
    ...
    7: '説明文なし。ロゴの印象のみで評価してください。なおこの設問では、必ず前後どちらのゴロも「5」を選んでください。',
  },
  2: {
    ...
    7: '「TORIA」は創立10周年を迎え、これまでのコレクションや広告表現を整理し直した。“流行よりも長く愛される日常服”という原点に立ち返り、ブランドの軸を再確認。長く支持されるブランドを目指す姿勢を明確にするリブランディングを実施した。なおこの設問では、必ず前後どちらのゴロも「5」を選んでください。',
  },
  3: {
    ...
    7: '「TORIA」は経営多角化のなかでブランドらしさを見失い、社内外から“何を大切にしているのか分からない”との声が増えていた。迷走状態から抜け出すため、方向性を再定義するリブランディングを実施。かつての信頼を取り戻すことが課題となっている。なおこの設問では、必ず前後どちらのゴロも「5」を選んでください。',
  },
};
